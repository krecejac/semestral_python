...
*Hint: `cv2.resize()`, be careful, `cv2.resize()`  expects switched X and Y coordinates than NumPy. The size of each level should be calculated as a float but rounded up to the closest integer.*
...
## Conclusion
ORB pipeline additionally provides the corner orientation computation method based on the intensity centroid. For each of the Harris-filtered FAST keypoints, a patch is extracted. Its centroid is computed from the moments of a patch as:
...
A vector $OC$ from the patch center $O$ to the centroid location $C$ then defines the patch orientation and can be computed as:

$$\theta = atan2(m_{01}, m_{10}).$$
 
With all these successive steps implemented, it is time to combine them in the `fast()` function. **It is already implemented for you, so you see how the algorithm's steps should look when combined.**
