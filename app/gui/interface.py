...
class App:
    """
    The class of the App. Handles all graphical interface things and passes important
    input values to the generator and utilities modules.
    """
    def __init__(self):
        # All of the modes
        data = (
        ...
        # Combobox
        self.cb = customtkinter.CTkComboBox(
            self.frame_cb,
            values=data,
            state='readonly',
    ...
    def Generate(self):
        ...
        """
        validated_entry = self.checkInput()
        util_d = {
            "Mode": self.var_cb.get(),
            "Width": int(validated_entry[0]),
            "Height": int(validated_entry[1]),
    ...
    def checkInput(self):
        width_validated = self.width_entry.get()
        height_validated = self.height_entry.get()
        if width_validated == '' or int(
                width_validated) == 0 or int(width_validated) > 2048:
            width_validated = 1024
        if height_validated == '' or int(
                height_validated) == 0 or int(height_validated) > 2048:
            height_validated = 1024
        return (width_validated, height_validated)
    ...
    def variableSetUp(self):
        ...
        # command handler for the entries
    ...
    def labelSetUp(self):
        self.main_label = customtkinter.CTkLabel(
            ...
            font=customtkinter.CTkFont(
                ...
                30,
                'bold'))
        self.label_cb = customtkinter.CTkLabel(
            self.frame_cb, text='Choose mode: ', width=15)
        self.label_entry_height = customtkinter.CTkLabel(
            self.frame_entries, text="Height:")
        self.label_entry_width = customtkinter.CTkLabel(
            self.frame_entries, text="Width:")
        self.label_slid_oct = customtkinter.CTkLabel(
            self.frame, text="Octaves")
        self.label_slid_per = customtkinter.CTkLabel(
            self.frame, text="Persistance")
        self.label_slid_lac = customtkinter.CTkLabel(
            self.frame, text="Lacunarity")
        self.label_val_oct = customtkinter.CTkLabel(
            self.frame, textvariable=self.var_octav)
        self.label_val_per = customtkinter.CTkLabel(
            self.frame, textvariable=self.var_persi_str)
        self.label_val_lac = customtkinter.CTkLabel(
            self.frame, textvariable=self.var_lacun_str)

    def sliderSetUp(self):
        self.slider_octav = customtkinter.CTkSlider(
            self.frame,
            from_=1,
            to=10,
            orientation=tk.HORIZONTAL,
            width=200,
            variable=self.var_octav,
            command=self.Update_oct)
        self.slider_persi = customtkinter.CTkSlider(
            self.frame,
            from_=0.1,
            to=1.0,
            number_of_steps=9,
            orientation=tk.HORIZONTAL,
            width=200,
            variable=self.var_persi,
            command=self.Update_per)
        self.slider_lacun = customtkinter.CTkSlider(
            self.frame,
            from_=0.1,
            to=3.0,
            orientation=tk.HORIZONTAL,
            number_of_steps=29,
            width=200,
            variable=self.var_lacun,
            command=self.Update_lac)

    def switchesSetUp(self):
        self.save_images = customtkinter.CTkSwitch(
            self.frame_switches, text="Save Images")
        self.custom_noise = customtkinter.CTkSwitch(
            self.frame_switches, text="Custom Noise", width=130)
        self.noise_3d = customtkinter.CTkSwitch(
            self.frame_switches,
            text="3D Plot",
            command=self.changeSwitch3DPlot)
        self.mapping = customtkinter.CTkSwitch(
            self.frame_switches,
            text="Mapping",
            width=130,
            command=self.changeSwitchMapping)
