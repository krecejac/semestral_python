...
def makeInteractiveGraph(elevation, utils):
    """
    ...
    filename = "utils/mapping/3d-terrain-plotly.html"
    html = plotly.offline.plot(fig2, filename=filename, include_plotlyjs='cdn')
...
def show3Dgraph(elevation: np.array, moisture: np.array, utils: var.Utils):
    ...
    if elevation.shape != utils.shape_im or moisture.shape != utils.shape_im:
        raise ValueError(
            "The generated noise is not in default width or height")
    fig = plt.figure()
    # Both surfaces have same x,y axis, they are the same lenght
...
def Dagobah(m):
    ...
    if m < 0.7:
        return var.Color.JUNGLE
    else:
        return var.Color.TUNDRA
...
def Kashyyyk(m):
    ...
    if m < 0.5:
        return var.Color.FOREST
    if m < 0.8:
        return var.Color.DENSE_FOREST
    else:
        return var.Color.SWAMP


def Kamino(e):
    # Favorizes water. Special mode.
    if e <= 0.5:
        return var.Color.DEEP_OCEAN
    if e <= 0.74:
        return var.Color.OCEAN
    if e <= 0.755:
        return var.Color.BEACH
    else:
        return var.Color.HIGH_MOUNTAIN


def Mustafar(e):
    # Special bonus mode. Uses lava as ocean and magma as low elevation values.
    if e <= 0.6:
        return var.Color.LAVA
    if e <= 0.65:
        return var.Color.MAGMA
    if e <= 0.74:
        return var.Color.BARREN
    else:
        return var.Color.HIGH_MOUNTAIN
...
def assignTerrain(e, m, threshold, mode):
    ...
    In edge cases, there is the threshold method used for determining simular biomes like Hoth 
    or Dagobah.
    The modes are: Terra, Dagobah, Tatooine, Hoth, Kashyyyk, Kamino, Mustafar
    """
    if mode == "Kamino":
        return Kamino(e)  # Special modes of Kamino and Mustafar
    if mode == "Mustafar":
        return Mustafar(e)  # Special modes of Kamino and Mustafar
    ...
    if e <= 0.68:
        if m < 0.6 + threshold:
            return var.Color.BEACH  # it's not impossible for Hoth to have beach
        elif mode == "Hoth":
            return var.Color.ICE
        else:
            return var.Color.SWAMP

    # Next is deciding if we are in mountains. Simular for each mode. Tatooine
    # doesnt have much snow.
    if e > 0.75:
        if m - threshold < 0.3:
            return var.Color.MOUNTAIN  # it's not impossible for Tatooine to have snow
        if m < 0.5:
            return var.Color.HIGH_MOUNTAIN
        else:
            return var.Color.SNOW
...
def perlinCustom(nx, ny, utils: var.Utils):
    ...
    if utils.octaves > 3:
        ...
        octaves = utils.octaves
    # fractal brownian motion uses octaves.
    for octave in range(octaves):
        e += (noiseRandom(nx * frequency, ny *
              frequency, utils.permutation) * amplitude)
        max_value += amplitude
...
def createElevation(y, utils: var.Utils) -> np.array:
    ...
    d = np.array(
        [((min(1, (np.power(nx, 2) + np.power(ny, 2)) / np.sqrt(2)) + 0.4)) for nx in row])
...
def createNoise(choice):
    ...
    if utils.mapping:
        makeInteractiveGraph(elevation, utils)
